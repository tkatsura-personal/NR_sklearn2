...
for option in possible_ML_options :
    ...
    sklearn_KNN.fit(x_train_bool, y_train_bool)
    y_pred = sklearn_KNN.predict(x_test_bool)
    #print(classification_report(y_test_bool, y_pred))

    sklearn_5fold_KNN = GridSearchCV(
    ...
    sklearn_5fold_KNN.fit(x_train_bool, y_train_bool)
    print(sklearn_5fold_KNN.best_params_)

    sklearn_KNN = KNeighborsClassifier(n_neighbors = sklearn_5fold_KNN.best_params_['n_neighbors'], 
                                    weights = sklearn_5fold_KNN.best_params_['weights'],
                                    p = sklearn_5fold_KNN.best_params_['p']) #Insert best parameters
    sklearn_KNN.fit(x_train_bool, y_train_bool)
    y_pred = sklearn_KNN.predict(x_test_bool)
    print(classification_report(y_test_bool, y_pred))
    print()