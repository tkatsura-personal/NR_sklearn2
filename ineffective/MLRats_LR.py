...
for option in possible_ML_options :
    ...
    sklearn_LR.fit(x_train_bool, y_train_bool)
    y_pred = sklearn_LR.predict(x_test_bool)
    #print(classification_report(y_test_bool, y_pred))

    sklearn_5fold_LR = GridSearchCV(
        LogisticRegression(),
        param_grid={
    ...
    sklearn_5fold_LR.fit(x_train_bool, y_train_bool)
    print(sklearn_5fold_LR.best_params_)

    sklearn_LR = LogisticRegression(C = sklearn_5fold_LR.best_params_['C'], 
                                    penalty = 'l2',
                                    solver = 'liblinear') #Insert best parameters
    sklearn_LR.fit(x_train_bool, y_train_bool)
    y_pred = sklearn_LR.predict(x_test_bool)
    print(classification_report(y_test_bool, y_pred))
