...
for option in possible_ML_options :
    ...
    sklearn_NN.fit(x_train_bool, y_train_bool)
    y_pred = sklearn_NN.predict(x_test_bool)
    #print(classification_report(y_test_bool, y_pred))

    sklearn_5fold_NN = GridSearchCV(
        MLPClassifier(),
        param_grid={
    ...
    sklearn_5fold_NN.fit(x_train_bool, y_train_bool)
    print(sklearn_5fold_NN.best_params_)

    sklearn_NN = MLPClassifier(hidden_layer_sizes = sklearn_5fold_NN.best_params_['hidden_layer_sizes'], 
                               solver = sklearn_5fold_NN.best_params_['solver']) #Insert best parameters
    sklearn_NN.fit(x_train_bool, y_train_bool)
    y_pred = sklearn_NN.predict(x_test_bool)
    print(classification_report(y_test_bool, y_pred))
    print()
